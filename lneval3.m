 function [model_outp,vaf1,vaf2,model_interm,sse2]=lneval3(inp,h,pm,nltype,outp,plots,centeroutputflag,h_len2)
...
for i=1:size(inp,2)
    ...
    %if h_len2 ~=0   %timeshift (advance) if there is acausal portion
       interm=conv([inp(1,i)*ones(h_len1,1);inp(:,i);inp(1,i)*ones(h_len2,1)],h); %convolution on padded input
