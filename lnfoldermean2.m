...
if odefitflag
    options_ode.modeltype=odemodeltype;  %'3o5gc';
   % [ltikernel,ltikerneltype,ltikernel_pm]=ltikernelfit(lnmstruct.h_est_causal(1:end-2/dt),modeltype,dt,odefitstarting);
    ...
    options_ode.plots=1;    
    lnmstruct.ode=odefit(lnmstruct,options_ode);
    ...
    lnmstruct.ode.lns=lnest9(meaninpq,meanoutp,h_len_trunc,svd_len,rng,1,0,detrendtype,nltype,0,iter,derivflag,crossvalopts);
end
...
if plots
    ...
    if odeplotflag
        ...
        plot(lnmstruct.ode.ltikerneltv,lnmstruct.ode.ltikernel,'g');
...
if ~subplotflag
    ...
    plot(mtv(lnmstruct.meaninp,dt),lnmstruct.meaninp-1,'r');
    hold on;
    plot(otv,lnmstruct.meanoutp);
    
    plot(otv,lnmstruct.simtrace);
...
if odefitflag
    if trialestflag
        ...
        for n=1:size(traces,2)
            lnmstruct.tracelns{n}.ode=odefit(lnmstruct.tracelns{n},options_ode);
